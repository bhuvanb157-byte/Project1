...
// Enable CORS so your front-end file can connect without security errors
app.use(cors());
app.use(bodyParser.json());

// Master database registry of Indian Government schemes
const schemesDatabase = [
    {
        title: "Startup India Seed Fund Scheme (SISFS)",
        desc: "Provides critical seed financing capital up to ₹20 Lakhs for validation of proof of concept, prototype engineering, and initial trial releases.",
    ...
    },
    {
        ...
        desc: "Offers straightforward collateral-free business loans up to ₹10 Lakhs. Special micro-interest concessions apply to woman entrepreneurs.",
    ...
    try {
        ...
        let matchedSchemes = schemesDatabase.filter(scheme => {
            ...
            if (scheme.isSpecialField) {
                return budgetMatch && (caste === "SC" || caste === "ST" || gender === "Female");
            }
            return budgetMatch && casteMatch;
        });

        // Add a completely dynamic state benefit template matching the exact client inputs
        matchedSchemes.push({
        ...
        // Return clean JSON array to the client front-end
...
// Run server
app.listen(PORT, () => {
    console.log(`🚀 udayamX Backend running securely on http://localhost:${PORT}`);
});
